...
def parse_data_packet(data, frame_id):
    ...
    if targets:
        ...
        for idx, target in enumerate(targets, start=1):        
            print(f"{Fore.GREEN if target['classification'] == "person" else Fore.BLUE if target['classification'] == "vehicle" else Fore.YELLOW}{idx:<8} {target['signal_strength']:<25} {target['range']:<15} {target['speed']:<25} {target['direction']:<15} {target['aizmuth_angle']:<25} {target['x']} {target['y']:<25} {target['latitude']:<25} {target['longitude']:<25} {target['classification']}")
